...
@before_after_equal
def rewards():
    return np.array([[0.0, 1.0, 3.0, 0.0, 0.0, 0.0, 1.0, 2.0],
...
def state_value_t_and_tp1():
    return np.array([[5.0, 5.0, 5.0, 5.0, 5.0, 5.0, 5.0, 5.0, 42.0],
...
def test_discounted_sum_rewards(rewards, dones, state_value_t, gamma):
    expected_discounted_return = np.array([[
        gamma * (1.0 + 3.0 * gamma), 1.0 + 3.0 * gamma, 3.0, 0.0, 0.0,
        gamma * (1.0 + gamma * 2.0), 1.0 + gamma * 2.0, 2.0
    ], [1.0, 0.0, 0.0, 0.0, 2.0, 1.0 + gamma * (gamma * 5.0), gamma * 5.0, 5.0]]).T
...
def test_td_target(rewards, dones, state_value_tp1, gamma):
    expected_td_target = np.array([[
        0.0 + gamma * 5.0, 1.0 + gamma * 5.0, 3.0 + gamma * 5.0, 0.0 + gamma * 5.0, 0,
        0.0 + gamma * 5.0, 1.0 + gamma * 5.0, 2
    ], [
        1, 0.0 + gamma * 5.0, 0.0 + gamma * 5.0, 0, 2, 1 + gamma * 5.0, 0.0 + gamma * 5.0,
        1 + gamma * 42.0
    ]]).T
...
def test_gae_estimation(rewards, dones, state_value_t, state_value_t_and_tp1, gamma,
                        gae_lambda):
    expected_td_target = np.array([[
        0.0 + gamma * 5.0, 1.0 + gamma * 5.0, 3.0 + gamma * 5.0, 0.0 + gamma * 5.0, 0,
        0.0 + gamma * 5.0, 1.0 + gamma * 5.0, 2
