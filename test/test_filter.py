...
def test_meanstd_filter_errors(num_features):
    filter_ = U.filter.MeanStdFilter(num_features=num_features)
...
def test_meanstd_filter_simple():
    filter_ = U.filter.MeanStdFilter(num_features=1, clip_range=np.inf)
...
def test_meanstd_filter_random(num_features, shift):
    ...
    filter_ = U.filter.MeanStdFilter(num_features=num_features, clip_range=np.inf)
